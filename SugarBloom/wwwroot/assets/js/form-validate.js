	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			 data: jQuery('form#CakeBake-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#CakeBake-contact').find('input,textarea').attr({value:''});
				jQuery('#CakeBake-contact').css({opacity:1});
				jQuery('#CakeBake-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#CakeBake-contact').animate({opacity:0.3});
		jQuery('#CakeBake-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#CakeBake-contact').find('input,textarea').css('border','none');
